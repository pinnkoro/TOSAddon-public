...
--	local RaidStance = tooltipframe:GetChild("RaidStance");
	...
	local token_pvpmine = tooltipframe:GetChild("token_pvpmine");
	token_pvpmine:ShowWindow(0)

	local token_event_coin = tooltipframe:GetChild("token_event_coin")
	token_event_coin:ShowWindow(0)
	...
	if ITEM_TOKEN == argNum then
		...
		if IS_MYPC_EXCHANGE_BENEFIT_STATE() == true then
			local tradeCountString = ScpArgMsg("AllowTradeByCount");
			-- .. " " .. tostring(accountObj.TradeCount)
		...
		token_pvpmine:ShowWindow(1)
		token_event_coin:ShowWindow(1)
...
-- Raid Glacier
function BUFF_TOOLTIP_GLACIER_LEGEND_WITCH_COLD_DEBUFF(buff, cls)
	if buff == nil then return ""; end
	local name = cls.Name;
	local over = buff.over;
	if over > 1 then
		name = name.."X"..over;
	end
...
function BUFF_TOOLTIP_ReduceDmgCommonAbil_Buff(buff, cls)
	local total_rate = buff.arg1
	local desc_txt = ScpArgMsg('ReduceDamageIncomeAbilDesc{rate}', 'rate', total_rate)
	return desc_txt, cls.Name
end
