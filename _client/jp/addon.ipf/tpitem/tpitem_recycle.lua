...
function CHECK_RECYCLE_SHOW_ITEM(frame, item, category)
	...
	if searchText ~= nil and searchText ~= '' then
		...
		itemName = string.lower(itemName); -- ?åÎ¨∏?êÎ°ú Î≥ÄÍ≤?
		searchText = string.lower(searchText); -- ?åÎ¨∏?êÎ°ú Î≥ÄÍ≤?
...
function RECYCLE_CREATE_SELL_LIST()
	...
	FOR_EACH_INVENTORY(invItemList, function(invItemList, invItem, retTable, mainSubGbox)
		...
		if invItem ~= nil then
			...
			if obj ~= nil then
				local ret, msg = IS_VALID_RECYCLE_ITEM(itemobj) 				
				if obj.SellPrice ~= 0 and ret == true then
...
function RECYCLE_DRAW_ITEM_DETAIL(obj, itemobj, itemcset, type, itemguid)	
	...
	--itemcset:SetUserValue("RCITEM_CLSID", tpitem_clsID); -- ?ÔøΩÍ±∞ ?ÔøΩÏöî??ÏßÄ  ?ÔøΩÏù∏??Ôø?
...
function TPSHOP_ITEM_RECYCLE_PREVIEW_PREPROCESSOR(parent, control, tpitemname, tpitem_clsID)
	...
	-- Í∏âÌïú?ÄÎ°?ÏΩîÏä§?¨Í≥º ?§Ïñ¥?ÖÏÑ∏Îß?ÏßÄ??Ï§? ÎØ∏Î¶¨Î≥¥Í∏∞ ?¨Î?Î•?TpItem???òÏ°¥?òÎäî Íµ¨Ï°∞Î•?Î∞îÍøî????
...
function RECYCLE_MAKE_TREE(frame)
	...
	-- TODO: Ï∂îÌõÑ Ïπ¥ÌÖåÍ≥†Î¶¨Î•??òÎ¶¥ ?åÏóê???¨Í∏∞ ?ÑÎûòÎ•??òÏ†ï?òÎ©¥ ?? ÏßÄÍ∏àÏ? Í≥†Ï†ï??Í≤ÉÎì§Îß??òÍ∏∞Î°??òÏ???
