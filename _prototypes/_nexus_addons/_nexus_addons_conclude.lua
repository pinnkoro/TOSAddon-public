...
function Battle_ritual_auto_buff_skill_start()
    local status, err = pcall(function() -- pcallでラップしてエラー落ちを防ぎつつログを出す
...
--[[function Battle_ritual_auto_buff_skill_start()
    local status, err = pcall(function()
        ...
        local skills_table = g.battle_ritual_settings.skills
        if not skills_table or next(skills_table) == nil then
            return
        end
        local skill_map = {}
        for i = 0, 40 - 1 do
            ...
            if slot_index then
                ...
                if slot and slot:GetIcon() then
                    ...
                    if cur_time == 0 then
                        local buff_id = data.buff_id
                        local need_buff = true
                        if buff_id > 0 and info.GetBuff(my_handle, buff_id) then
                            need_buff = false
                        end
                        if need_buff then
                            table.insert(g.battle_ritual_use_queue, {
...
function Battle_ritual_auto_buff_skill(_nexus_addons)
    ...
    local next_info = g.battle_ritual_use_queue[1]
    local my_handle = session.GetMyHandle()
    local buff_info = nil
    if next_info.buff_id and next_info.buff_id > 0 then
        buff_info = info.GetBuff(my_handle, next_info.buff_id)
    end
    local current_cd = ICON_UPDATE_SKILL_COOLDOWN(next_info.icon)
    if current_cd == 0 and (next_info.buff_id == 0 or not buff_info) then
        ICON_USE(next_info.icon)
        table.remove(g.battle_ritual_use_queue, 1)
        return 1
    end
    table.remove(g.battle_ritual_use_queue, 1)
...
function Indun_panel_create_common_buttons(indun_panel)
    ...
    if g.settings.indun_list_viewer.use == 1 then
        lbtn_action = "indun_list_viewer_title_frame_open"
        table.insert(tooltip_parts, "Left-Click: [ILV] Open")
    elseif g.settings.instant_cc.use == 1 then
        lbtn_action = "INSTANTCC_APPS_TRY_MOVE_BARRACK"
        table.insert(tooltip_parts, "[InstantCC] Open")
    end
    if g.settings.other_character_skill_list.use == 1 then
            ...
            if not inv_item.isLockState then
                ...
                if life_time > 0 then
                    table.insert(candidate_tickets, {
                        use_item = inv_item,
                        priority = (life_time < 86400) and 1 or 2
                    })
                else
                    use_item = inv_item
                end
            else
                local item_cls = GetClassByType("Item", classid)
                ui.SysMsg(ClMsg("MaterialItemIsLock") .. " (" .. item_cls.Name .. ")")
            end
        end
    end
